...
cursor = conn.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS cpu_load
                  (id INTEGER PRIMARY KEY AUTOINCREMENT,
                   timestamp DATETIME,
                   load FLOAT)''')
cursor.execute('''CREATE TABLE IF NOT EXISTS cpu_average
                  (id INTEGER PRIMARY KEY AUTOINCREMENT,
                   timestamp DATETIME,
                   average FLOAT)''')
conn.commit()
...
def save_cpu_load():
    while True:
        ...
        cursor.execute('''INSERT INTO cpu_load (timestamp, load)
                          VALUES (?, ?)''', (timestamp, cpu_load))
        conn.commit()
        time.sleep(5)

def save_cpu_average():
    while True:
        cursor.execute('''SELECT AVG(load)
                          FROM cpu_load
        ...
        timestamp = datetime.now()
        cursor.execute('''INSERT INTO cpu_average (timestamp, average)
                          VALUES (?, ?)''', (timestamp, cpu_average))
        conn.commit()
        time.sleep(60)
